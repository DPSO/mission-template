...
if ((headgear player in _gear||goggles player in _goggles )&&(vehicle player == player)) then
      {
...
if (( vehicle player) isKindOf "Air" && ((getPosASLVisual (vehicle player) select 2) > 4000)&&(!((headgear player in _gear)||(goggles player in _goggles )))) then {
...
if ((vehicle player == player)&&(!((headgear player in _gear)||(goggles  player in _goggles )))&&(( getPosASL (vehicle  player) select 2) > 4000 ))then {
...
if (((headgear player in _gear)||(goggles player in _goggles ))or( getPosASL (vehicle  player) select 2) < 3999 )  then {
...
if ((vehicle player == player)&&(!((headgear player in _gear)||(goggles player in _goggles )))&&(( getPosASL (vehicle  player) select 2) > 4000 ) )then {
   [player, 0.8, "chest", "unknown"] call ace_medical_fnc_addDamageToUnit;
   [player, true, 0, true] call ace_medical_fnc_setUnconscious;
};
   sleep 1;
};
